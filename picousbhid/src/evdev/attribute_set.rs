...
pub struct AttributeSetRef<T> {
    _indexer: core::marker::PhantomData<T>,
...
macro_rules! evdev_enum {
    ...
    (
        ...
        impl $crate::evdev::attribute_set::ArrayedEvdevEnum for $t {
            type Array = $Array;
    ...
    ($t:ty, $($(#[$attr:meta])* $c:ident = $val:expr,)*) => {
        ...
        impl core::str::FromStr for $t {
            type Err = crate::evdev::EnumParseError;
            ...
            fn from_str(s: &str) -> Result<Self, Self::Err> {
                ...
                match map.iter().find(|e| e.0 == s) {
                    Some(e) => Ok(e.1),
                    None => Err(crate::evdev::EnumParseError(())),
                }
            }
        }
        ...
        impl $crate::evdev::attribute_set::EvdevEnum for $t {
            #[inline]
            fn from_index(i: usize) -> Self {
                Self(i as _)
            }
